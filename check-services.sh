...
echo "==> Volumes"
for v in srcs_mariadb_data srcs_wordpress_data; do
...
echo "==> Network"
if docker network ls --format '{{.Name}}' | grep -qx inception_network; then
	ok "inception_network exists"
else
	fail "inception_network missing"
fi
